...
## Methods
All clusters were then automatically annotated for their most dominant cell types by using the *SingleR* package (v2.8.0), both at the cell level and the cluster level ().  As a reference dataset, the MouseRNAseqData was used from the *celldex* package (v1.16.0) ().  First, to explore all the different cell types across the entire cell community, 
